...
| mcp | `mcp/` | unit + MCP protocol (API stubbed) | vitest | `mcp.yml` | no |
...
**mcp** — the stdio MCP server: output formatting, ref resolution + API error
mapping (stubbed `fetch`), the blocking-run loop with injected fakes, and the
protocol surface via the SDK's `InMemoryTransport` (tool list, annotations,
`tools/list` size budget). No running stack.
