...
const EnvSchema = z.object({
  ...
  // Local-first tool with zero authentication by design (LocalNoAuthProvider) —
  // default to loopback-only so the API isn't reachable from the network unless
  // explicitly opted into (e.g. accessing the studio from another device).
...
export type AppConfig = {
  databaseUrl: string;
  apiPort: number;
  /** Interface the API server binds to. Default '127.0.0.1' — loopback-only,
   *  since there's no authentication (see AuthProvider). Set API_HOST=0.0.0.0
   *  to opt into LAN/container reachability. */
  apiHost: string;
  webPort: number;
